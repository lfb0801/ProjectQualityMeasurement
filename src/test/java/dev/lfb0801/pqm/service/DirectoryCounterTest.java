...
class DirectoryCounterTest {
    @Test
    void countLinesInThisProject() throws IOException, URISyntaxException {
        String path = getProjectRoot();

        DirectoryCounter directoryCounter =
                new DirectoryCounter(path);

        Map<Language, Counts> sources = directoryCounter.matchPattern("**/src/main/**");
        Map<Language, Counts> tests = directoryCounter.matchPattern("**/src/test/**");
        ...
        assertThat(javaSource)
                .matches(c -> c.getCommentLines() == 0, "Source code should not have comments")
