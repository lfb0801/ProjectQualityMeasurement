...
class LOCScannerTest {
    ...
    void countLinesInThisProject() throws IOException {
        ...
        assertThat(sources).matches(c -> c.getCommentLines() == 0, "Source code should not have comments")
                           .matches(c -> c.getBlankLines() < c.getCodeLines(),
                                    "There should not be more blank than source lines"
                           )
                           .matches(c -> c.getCodeLines() < tests.getCodeLines(),
                                    "Should have more test than source code"
                           );
    }
